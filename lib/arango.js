...
Arango.prototype.create = function (model, data, callback) {
    ...
    this._client.document.create(true, model, data, function(err, res, hdr) {
        //console.log("CREATE RESULT", err, res);
...
Arango.prototype.all = function(model, filter, callback) {
    ...
    var resultFunction = function(err, res, hdr) {
        //console.log(res);
        callback(err ? true : false, err ? res : res.result.map(function(o) { o.id = o._id; delete o._id; return o; }));
    };
    if (filter) {
        if (filter.where) {
            var filterQuery = [];
    ...
    //console.log(this._client.query.string, queryArgs);
